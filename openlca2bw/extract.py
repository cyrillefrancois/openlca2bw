...
class Extraction_functions():
        ...
        for u_group in units:
            for u in return_attribute(u_group,'units'):
                if  return_attribute(u_group,'defaultFlowProperty'):
                    ...
                    ref_u =  normalize_units([return_attribute(uni,'name') for uni in return_attribute(u_group,'units') if return_attribute(uni,'referenceUnit')][0]) 
        ...
        for method in methods:
            ...
            for c in impact_categories:
                ...
                list_methods.append({
                    ...
                    'ref_unit': return_attribute(cate,'referenceUnitName'),
                    'list_cf': list_cf
                    })
        print(pbar)
        return(list_methods)  
    ...
    def build_exchange_dict(self, exchange, process, database_ids_dic, exclude_S):
        ...
        if return_attribute(exchange,('flow','flowType')) == 'ELEMENTARY_FLOW':
            if return_attribute(exchange,'input') == False and return_attribute(exchange,('flow','categoryPath'))[1] == 'Resource':
                cf_IO = -1
            if return_attribute(exchange,'input') == True and return_attribute(exchange,('flow','categoryPath'))[1] != 'Resource':
                cf_IO = -1
            ...
            if return_attribute(exchange,('flow','flowType')) == 'WASTE_FLOW':
                cf_IO = -1
            if return_attribute(exchange,'avoidedProduct'):
                cf_IO = -cf_IO
